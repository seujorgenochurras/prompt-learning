...
public final class ConsoleListBuilder implements CommandConsoleBuilder {
    ...
    public List<PromptableElementIF> build() {
        ListChoice buildResult = new ListChoice(listMessage, listName, pageSize, pageSizeType,
            parseChoicesListToItemList());
        return List.of(buildResult);
    }

    private List<ListItemIF> parseChoicesListToItemList() {
        return availableChoices.stream()
            .map(choice -> new ListItem(choice.name, choice.value))
