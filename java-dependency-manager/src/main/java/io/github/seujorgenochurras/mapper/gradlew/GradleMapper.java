...
public class GradleMapper extends DependencyMapper {
    ...
    protected List<Plugin> getAllPluginDeclarations() {
        GradleValidatorChain<String> pluginDeclarationValidator = GradleValidatorChain.initialValidator(
            new StringValidator())
            .addValidator(new StringHasSizeGreaterThan(5))
            .addValidator(new StringStartsWithRegex("id"));

        List<Plugin> plugins = new ArrayList<>();
        gradleForest.getTreeByName("plugins")
            .getNodes()
            .stream()
            .filter(possiblePluginDeclarationNode -> pluginDeclarationValidator.validate(possiblePluginDeclarationNode
                .getTextContents()))
            .forEach(pluginDeclarationNode -> plugins.add(StringToPluginParser.stringToPlugin(pluginDeclarationNode
                .getTextContents())));
        return plugins;
    }
    ...
    protected List<Dependency> getDependencyDeclarations() {
        ...
        GradleValidatorChain<String> dependencyDeclarationValidator = GradleValidatorChain.initialValidator(
            new StringValidator())
        ...
        gradleForest.getTreeByName("dependencies")
            .getNodes()
            .stream()
            .filter(probableDependencyDeclarationNode -> dependencyDeclarationValidator.validate(
